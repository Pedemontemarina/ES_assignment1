...
static circular_buffer_t rx_buf = {.head = 0, .tail = 0, .size = RX_BUFFER_SIZE}; // RX ring buffer
static circular_buffer_t tx_buf = {.head = 0, .tail = 0, .size = TX_BUFFER_SIZE}; // TX ring buffer
...
_U1TXInterrupt(void) {
    ...
    if (tx_buf.tail != tx_buf.head) {         // If TX buffer has data
